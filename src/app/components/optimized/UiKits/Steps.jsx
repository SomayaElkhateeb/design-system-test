...
const Steps = ({ stepsContent, onNext, onBack, onFinish, currentStep }) => {
		...
		<div className='flex flex-col space-y-4'>
			<div className='flex justify-between'>
				...
				<div className='flex space-x-2'>
					{currentStep > 1 && (
						<button disabled={currentStep === 1} onClick={onBack}>
							<FaArrowLeft />
						</button>
					)}
					{currentStep < stepsContent.length && (
						<button disabled={currentStep === stepsContent.length} onClick={onNext}>
							<FaArrowRight />
						</button>
					)}
