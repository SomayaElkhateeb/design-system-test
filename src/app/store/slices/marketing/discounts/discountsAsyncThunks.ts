...
import MainApi, { baseUrl } from 'src/app/api/MainApi';
import { DiscountInterface } from 'src/app/interface/DiscountInterface';

export const getDiscounts = createAsyncThunk<DiscountInterface[], { rejectValue: string }>(
	'discount/getDiscounts',
	async (_, thunkAPI) => {
		const { rejectWithValue } = thunkAPI;
		try {
			const { data } = await MainApi.get<DiscountInterface[]>(`${baseUrl}/discount`);
			return data;
		} catch (error) {
