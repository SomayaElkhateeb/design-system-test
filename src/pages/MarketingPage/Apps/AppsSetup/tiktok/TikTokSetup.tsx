...
const TikTokSetup: React.FC<TikTokSetupProps> = ({ platform }) => {
	...
	const { mega_title, tikTok_settings }: TikTokTabs | null = getTikTokSetup(platform);
	...
	return (
		<section className='w-full p-5'>
			<div className='w-2/5 m-auto'>
				...
				<div className='p-3'>
					<p>
						{privacy_notice.text} <span className='cursor-pointer text-primary'>{privacy_notice.linkText}</span>
					</p>
				</div>

				<div className='flex justify-end mt-8 space-x-3'>
					<Button variant='secondaryBtn' onClick={() => navigate('/apps/tiktok')}>
						Discard
					</Button>
					<Button onClick={() => setSearchParams({ add_channel: 'true' })}>Add Sales Channel</Button>
				</div>

				<div className='p-3 m-auto text-center'>
					<p>
						{agreement_terms.text} <span className='cursor-pointer text-primary'>{agreement_terms.linkText}</span>
					</p>
				</div>
			</div>
		</section>
