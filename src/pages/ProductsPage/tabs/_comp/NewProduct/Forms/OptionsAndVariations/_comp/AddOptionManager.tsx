...
import { AttributeInterface, optionInterface } from 'src/app/interface/AttributeInterface';
...
export default function AddOptionManager<TFormStore>(props: Props<TFormStore>) {
	...
	const [attributeValue, setAttributeValue] = useState<AttributeInterface[]>([]);
	...
	const handelAttributeValueComponent = (i: number) => {
		let attributeValue = attributes?.filter((e) => e?.code?.toString() === props.formStore.watch(`variants[${i}].code`)?.toString())
				...
				<div className="global-cards my-2">
					<DropDownMenu addCompo={
						...
						<div className='grid grid-cols-1 gap-4  '>
							...
							<div className="mt-2">
								{handelAttributeValueComponent(i)}
							...
							<div className="mt-2">
								<FormField
									...
									render={(field) => (
										<CheckBox
											label={t('Status')}
											checked={props.formStore.watch(`variants[${i}].status`) === 1 ? true : false}
